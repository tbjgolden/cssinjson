 ...
 */
export const compile = (input, vars, root) => generate(expand(input, vars, root));
 ...
 */
export const expand = (input, vars, root) => {
  vars = vars || {};
  vars._ = vars._ || [];
 ...
 */
export const generate = flatRules => (
  flatRules
    .map(pair => `${pair[0]} { ${
      pair[1]
        .map(pair => (
 ...
 */
export const inject = (value, vars, root) => {
  vars = vars || {};
  vars._ = vars._ || [];
  root = type(root, 'b') ? root : true;
